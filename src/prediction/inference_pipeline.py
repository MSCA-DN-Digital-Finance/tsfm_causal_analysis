...
def inference_pipeline(model_name: str, model_spec: dict, data: np.ndarray, horizon: int) -> np.ndarray:
    """
    ...
    # For Moirai we need to feed horizon into loader and not inference_fn
    if model_name == "moirai":
        
        model = model_spec["loader"](horizon=horizon)

        formatted_in = model_spec["input_adapter"](data)

        raw_out = model_spec["inference_fn"](model, formatted_in)
